package br.com.microservice.token.filter;
...
import br.com.microservice.token.util.SecurityContextUtil;
...
public class JwtTokenAuthorizationFilter extends OncePerRequestFilter {
    ...
    @Override protected void doFilterInternal(@NonNull HttpServletRequest request, @NonNull HttpServletResponse response, @NonNull FilterChain chain) throws ServletException, IOException {
        ...
        SecurityContextUtil
                .setSecurityContext(equalsIgnoreCase("signed", this.jwtConfiguration.getType()) ? validate(token) : decryptValidating(token));
        chain.doFilter(request, response);
    }
